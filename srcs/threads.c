...
/*   Updated: 2020/03/07 16:14:27 by aboitier         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
t_complex	init_complex(int x, int y, t_info *info)
{
	...
	nb.c_r = x / info->zoom + info->boundaries.x1;
	nb.c_i = y / info->zoom + info->boundaries.y1;
	nb.z_r = 0;
	nb.z_i = 0;
	nb.i = 0;
	return (nb);
...
t_complex	init_j_complex(int x, int y, t_info *info)
{
	...
	nb.z_r = x / info->zoom + info->boundaries.x1;
	nb.z_i = y / info->zoom + info->boundaries.y1;
	nb.i = 0;
	return (nb);
}
...
void		pthread_init(t_info *info)
{
	...
	while (i < NB_THREADS)
	{
		...
		pthread_create(&thrtab[i], NULL, (void *)draw_fracts, &thread_info[i]);
		i += 1;
	}
	while (i--)
		pthread_join(thrtab[i], NULL);
	mlx_put_image_to_window(info->mlx_ptr,
					info->window_ptr, info->img_ptr, 0, 0);
}
